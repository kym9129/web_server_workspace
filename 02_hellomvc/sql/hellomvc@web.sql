...
create table member(
    ...
    hobby varchar2(200), --운동,등산,독서,게임,여행 (체크박스)
...
--멤버검색 페이징 (searchPagedMember)
select*
from(
    select row_number() over(order by enroll_date desc) rnum,
            M.*
    from member M
    where member_id like '%a%') --where이하 #처리 후 replace
where rnum between 11 and 20;

--totalContents (searchMemberCount)
select count(*) count
from (
    select M.*
    from member M
